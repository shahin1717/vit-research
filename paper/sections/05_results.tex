...
$t = 0.48$, $p = 0.681$, with a spread three times the effect. The two arms that
...
\subsection{H1: Generalization Gap Narrows on Validation Only}
...
$K = 0, 1, 4, 8$), showing a narrow spread of $0.005$ against a within-arm standard deviation of
...
$2.17$ pp in the control arm, against $0.24$ pp on the test split, representing a factor of
...
in 3 of 3 seeds, $t = 8.37$, $p = 0.014$, representing the smallest $p$-value in the study and the only one below $0.05$ on a test-split quantity. It also has the lowest
...
depth-dependent and non-monotonic (reaching a minimum of $3.34$ bits at block 2, a
maximum of $6.27$ bits at block 5, and declining through the final third), but
...
differences are positive for every register arm, showing a slight increase rather than a decrease in outlier frequency, though none approaches significance
...
$44.7$ for $K = 0, 1, 4, 8$), indicating that no arm was still improving materially at the
...
\subsection{Data Budget Scaling: Scale Invariance at 300 Images/Class}
\label{sec:scaling}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{accuracy_100_vs_300.pdf}
...
Table~\ref{tab:main_results} (Panel B) and Fig.~\ref{fig:accuracy_scaling} present the comparative scaling outcomes. As expected under standard empirical scaling laws, tripling the available data substantially improves representation quality across all arms: baseline test accuracy increases by $+6.14$ percentage points ($75.19 \pm 0.24\% \to 81.33 \pm 0.37\%$), while validation loss drops from $1.6829$ to $1.4203$.

Crucially, the relative ordering and comparative efficacy of register tokens remain unchanged:
\begin{enumerate}
\item \textbf{No Top-1 Accuracy Gain:} The register-free control ($K=0$) maintains the highest mean test accuracy at $81.33 \pm 0.37\%$. Both $K=1$ ($81.23 \pm 0.19\%$) and $K=4$ ($81.23 \pm 0.18\%$) perform marginally below control, while $K=8$ remains the lowest-performing arm ($81.02 \pm 0.45\%$).
\item \textbf{Lack of Paired Consistency:} In paired per-seed analysis, registers gain nothing: $K=1$ and $K=4$ win on only 1 of 3 seeds ($\bar{\delta} = -0.10$ pp), while $K=8$ loses on 3 of 3 seeds ($\bar{\delta} = -0.31$ pp). None of the differences reach statistical significance ($p > 0.50$).
\item \textbf{Entropy Uniformity:} Layer-11 Shannon attention entropy remains uniform across configurations ($5.002$ bits for $K=0$ versus $4.910$, $4.933$, and $4.843$ bits for $K \in \{1, 4, 8\}$).
\end{enumerate}
